...
class MakeTheme extends Command
{
    ...
    {
        $this->themePath = $this->config->get('themes-manager.directory', 'amuz-themes');

        $this->sectionMessage('Create Theme for AMUZ CMS', 'Create new Theme');
        if ($this->validateName()) {
            ...
            try {
                ...
                $this->alert('Theme successfully created');
                $this->warn('생성된 테마는 아래 명령을 통해 Git 저장소를 등록해야합니다.');
                $this->comment("https://github.com/organizations/amuzcorp/repositories/new");
                $this->comment("위 주소로 접속하여 {$this->theme['name']} 이름으로 새 저장소를 생성합니다.");
                $this->comment('생성이 완료되면 아래 명령을 통해 저장소와 연결할 수 있습니다.');
                $this->info("cd amuz-themes/{$this->theme['vendor']}/{$this->theme['name']} && git init && git add . && git commit -m 'first commit' && git branch -M main && git remote add origin https://github.com/amuzcorp/{$this->theme['name']}.git && git push -u origin main && cd ../../../");
                $this->comment("이후에는 테마에 포함된 README.md를 참고하여 적절히 활용할 수 있습니다.");
    ...
    private function generateTheme(): void
    {
        ...
        $source = app_path('Stubs/_theme-structure');
